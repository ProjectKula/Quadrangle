...
export async function requestEmailCode(id: string, email: string) {
    const response = await fetch(`${getRoot()}/v0/auth/signup`, {
        method: 'POST',
        headers: {
...
export async function sendEmailCode(token: string, code: string) {
    const response = await fetch(`${getRoot()}/v0/auth/signup/code`, {
        method: 'POST',
        headers: {
...
export async function setInitialCredentials(token: string, password: string) {
    const response = await fetch(`${getRoot()}/v0/auth/signup/cred`, {
        method: 'POST',
        headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
